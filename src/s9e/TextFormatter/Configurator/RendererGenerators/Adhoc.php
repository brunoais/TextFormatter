...
abstract class Adhoc
{
	...
	*/
	public static function generate($xsl)
	{
		...
		$php = '{
			protected $inElement;
			protected $xpath;
			protected function closeElement()
			{
				if ($this->inElement)
				{
			...
			protected function at($root, $xpath = "*")
			{
				if ($root->nodeType === 3)
				{
					$this->closeElement();
				...
				{
					$nodes = ($xpath === "*") ? $root->childNodes : $this->xpath->query($xpath);
		...
		foreach ($dom->getElementsByTagNameNS('http://www.w3.org/1999/XSL/Transform', 'template') as $node)
		{
			...
			$conditions = array();
			foreach (explode('|', $match) as $tagName)
			{
				$conditions[] = "\$node->localName==='" . $tagName . "'";
			}

			$php .= $else . 'if(' . implode('||', $conditions) . '){?>';
	...
	protected static function writeElement(DOMNode $node)
	{
		...
		foreach ($node->attributes as $attribute)
		{
			...
			$php .= preg_replace_callback(
				...
				function ($m)
				{
					return $m[1] . '<?php echo $this->xpath->evaluate(' . self::export($m[2]) . ',$node)?>';
				},
				$attribute->value
			);
			$php .= '"';
		...
		{
			// TODO: html vs xhtml
			if (!preg_match('/^(?:area|base|br|col|command|embed|hr|img|input|keygen|link|meta|param|source|track|wbr)$/D', $node->localName))
			{
				$php .= '/';
			}

			$php .= '><?php $this->inElement=false?>';
		}
	...
	*/
	protected static function elementCopyOf(DOMNode $node)
	{
		...
		if (preg_match('#^\\s*@([-a-z0-9_]+)\\s*$#D', $expr, $m))
		{
			return "<?php if (\$node->hasAttribute('" . $m[1] . "')){?> " . $m[1] . '="<?php echo $node->getAttribute(\'' . $m[1] . '\');}?>"';
		}
